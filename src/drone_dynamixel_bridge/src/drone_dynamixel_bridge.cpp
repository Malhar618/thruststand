...
const char* DEVICENAME = "/dev/ttyUSB1"; // Verify this port
...
const double CURRENT_STEP = 0.00269; // 2.69 mA per unit
const double DEFAULT_CURRENT_LIMIT_STEPS = 1193.0; // ~3.21 A

// --- Filter Differentiator Class --- (for obtaining filtered derivative)
class FilterDiff {
public:
...
public:
  DroneDynamixelBridgeNode()
    : Node("drone_dynamixel_bridge_node"),
      // Inertia tensors for the UAV and components (expressed in the appropriate frame)
  ...
  ~DroneDynamixelBridgeNode() {
    ...
    // Disable torque on all Dynamixels without checking port open state (since isOpen() is unavailable)
...
private:
  ...
  // C_J_I_ is declared only once (here in the private section if needed for calculations)
  ...
  void timer_callback() {
    ...
    // Acquire odometry and angular velocity safely
    ...
    // Get rotation matrix from odometry quaternion ([w, x, y, z])
    ...
    // Initialize filters on first valid cycle
    if (first_run_) {
    ...
    // Calculate Euler rates (for logging purposes)
    calculateEulerRates(phi_raw_, theta_raw_, p_raw_, q_raw_, r_raw_,
                        phi_dot_raw_calc_, theta_dot_raw_calc_, psi_dot_raw_calc_);

    // Update DCMs for additional coordinate transformation if necessary
    update_dcms(phi_raw_, theta_raw_, psi_raw_);

    // Compute torques for each component using our simplified method
    ...
    Eigen::Vector3d required_motor_torque = -total_inertial_torque; // Cancel inertial torque

    // Compute inverse Jacobian for control allocation based on raw phi and theta
  ...
  // Helper: Extract Euler angles (ZYX convention) from rotation matrix
  void extractEulerAnglesZYX(const Eigen::Matrix3d& R, double &phi, double &theta, double &psi) {
    theta = -asin(R(0,2)); // From R(0,2) = -sin(theta)
    theta = std::max(-M_PI_2, std::min(M_PI_2, theta)); // Clamp theta
  ...
  // Helper: Calculate Euler rates from body rates (p, q, r) and Euler angles (phi, theta)
  ...
  void update_dcms(double phi, double theta, double /*psi*/) {
    ...
    // DCM for pitch ring (Rotation about X)
    C_J_LPR_ << 1, 0, 0,
                0, cphi, sphi,
                0, -sphi, cphi;
    // DCM for yaw ring (from provided formulation)
  ...
  // Helper: Convert torque (Nm) to Dynamixel current steps
  ...
  // Helper: Write a single byte to a Dynamixel using the SDK
  void write_byte_with_log(uint8_t id, uint16_t addr, uint8_t val, const std::string &op) {
    ...
    if (res != COMM_SUCCESS) {
      RCLCPP_ERROR(this->get_logger(), "ID:%d Addr:%d (%s) Comm Fail: %s", id, addr, op.c_str(),
                   packetHandler_->getTxRxResult(res));
    } else if (err != 0) {
      RCLCPP_ERROR(this->get_logger(), "ID:%d Addr:%d (%s) DXL Error: %s", id, addr, op.c_str(),
                   packetHandler_->getRxPacketError(err));
    }
  }

  // Helper: Send current commands via GroupSyncWrite
  bool send_dynamixel_commands(int32_t current_roll, int32_t current_pitch, int32_t current_yaw) {
    ...
    if (!tx_ok) {
      RCLCPP_ERROR_THROTTLE(this->get_logger(), *this->get_clock(), 1000, "SyncWrite fail: %s",
                             packetHandler_->getTxRxResult(res));
    }
    groupSyncWrite_->clearParam();
    return tx_ok;
  ...
  // Helper: Dynamixel initialization using the SDK
  bool initialize_dynamixels() {
    ...
    uint8_t op_mode = 0; // Set Current Control mode
    for (uint8_t id : DXL_IDS) {
