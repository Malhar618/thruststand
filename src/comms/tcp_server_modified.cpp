...
void start_pixhawk_flightstack() {
    char command1[255];
    char command2[255];                                                                                                     sprintf(command1, "sh -c ./start_uxrce.sh");
    usleep(10000);
...
int main() {
	...
	start_pixhawk_flightstack();
        send_file(client_socket, L2filename);
        //send_file(client_socket, L2filenameMod2);
        //send_file(client_socket, L2filenameMod3);
    //}
    close(client_socket);
    close(server_fd);
